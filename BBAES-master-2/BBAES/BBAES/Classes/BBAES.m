...
+ (NSData *)encryptedDataFromData:(NSData *)data IV:(NSData *)iv key:(NSData *)key options:(BBAESEncryptionOptions)options
{
	...
	if (options & BBAESEncryptionOptionsIncludeIV) {
        NSMutableData *saltData = [saltedKey mutableCopy];
        [saltData appendData:iv];
		NSMutableData *mutableData = [NSMutableData dataWithData:saltData];
...
+ (NSData *)decryptedDataFromData:(NSData *)data IV:(NSData *)iv key:(NSData *)key
{
	...
	if (!iv) {
        ...
        saltData = [data subdataWithRange:NSMakeRange(0, ivLength)];
        iv = [data subdataWithRange:NSMakeRange(ivLength, ivLength + ivLength)];
		encryptedData = [data subdataWithRange:NSMakeRange(ivLength + ivLength, [data length] - (ivLength + ivLength))];
        NSLog(@"%@", encryptedData);
	} else {
		encryptedData = data;
	}
